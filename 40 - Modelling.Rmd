...
I am putting the assert statement for only two of the columns from the windorizing1. There are a lot of columns in the dataset on which the function can be applied. I have kept it to a minimum in order to not cause cnfusion amongst people
...
ufc_refined_modelling <- ufc_refined %>% 
  ...
  Day_of_Week(date,'Year','Month','Day_of_Week') %>% 
  select(winner_onehot_encode1,date,Year,Month,Day_of_Week,contains("windorized_values")) %>% 
  assert(in_set(0,1,2), winner_onehot_encode1) %>% 
   assert(within_bounds(0,26),b_avg_body_att_windorized_values) %>% 
  assert(within_bounds(0,250),b_avg_clinch_landed_windorized_values) %>% 
  assert(within_bounds(0,2020),Year) %>% 
  assert(within_bounds(1,12),Month) %>% 
  assert(within_bounds(1,7),Day_of_Week)
  

ufc_refined_modelling
